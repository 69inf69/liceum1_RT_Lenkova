...
class WeatherDiary:
    ...
    def create_widgets(self):
        ...
        ttk.Combobox(self.root, textvariable=self.precipitation_var,
        ...
        tk.Button(self.root, text="Сохранить в JSON", command=self.save_data).grid(
            row=9, column=0, pady=5)
        tk.Button(self.root, text="Загрузить из JSON", command=self.load_data).grid(
    ...
    def update_table(self):
        ...
        for record in self.records:
            self.tree.insert("", "end", values=(
                ...
                record["description"], record["precipitation"]
            ))
